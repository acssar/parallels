...
int main(void)
{
    ...
    *err = 1;
    ...
    while(it < max_it && *err > 1e-6)
    {
        ...
        calc_heat_equation<<<GRID_SIZE, BLOCK_SIZE>>>(U_d, U_d_n, N); //запуск ядер

        if(it % 100 == 0)
        {

            *err = 0;
            calc_diff<<<GRID_SIZE, BLOCK_SIZE>>>(U_d, U_d_n, U_d_diff, N); //высчитывание массивов разниц
            cub::DeviceReduce::Max(d_temp_storage, temp_storage_bytes, U_d_diff, d_err, N*N); //нахождение максимальной разности
